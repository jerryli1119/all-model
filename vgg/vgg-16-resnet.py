...
def resblock(name, l_input, w, b):
    out=tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(l_input, w, strides=[1, 1, 1, 1], padding='SAME'),b))  
...
def convLevel(i,input,type):  
    ...
    with tf.name_scope('conv'+str(num)):
        ...
        if  type=='p':  
            out1 = resblock('conv'+str(num),out,weights['wc'+str(num)],biases['bc'+str(num)])
            #ou =tf.concat([out, out], axis=-1, name="concat_{}".format("test"))
            return out+out1
        return out
