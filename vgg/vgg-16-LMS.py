...
weights ={  
    ...
    'wc2':tf.Variable(tf.random_normal([3,3,32,128])),  
    'wc3':tf.Variable(tf.random_normal([3,3,128,128])),  
    'wc4':tf.Variable(tf.random_normal([3,3,128,512])),  
    ...
    'wd3':tf.Variable(tf.random_normal([4096,4096])), 
...
def main(iternum,gpu_size):
  ...
  #test LMS gpu usage 	0.45		- 16	- chain_rule
  #test LMS gpu usage 	0.45		- 16	- direct_order
...
# RECOMPUTATION_HEURISTICS SWAPPING_HEURISTICS NO_MEM_OPT ITRI_SWAPPING
  ...
  #with tf.Session() as sess:
    ...
    while step < iternum:  
      ...
      #summary_writer = tf.summary.FileWriter("./test2", graph=tf.get_default_graph())
